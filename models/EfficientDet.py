...
from models.FPN import PyramidFeatures, BiFPN
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, backbone, neck, head, num_classes):
        ...
        self.extras1 = nn.Sequential(
                ...
                BasicConv(128, 128, kernel_size=3, stride=1, padding=0) # 1 * 1
        )
    ...
    def forward(self, x):
        ...
        features.append(self.extras1(features[-1]))
...
def build_net(phase, size=300, num_classes=21):
    ...
    backbone = VGG()
    BiFPN_inputs = [128, 128, 128, 256, 256]
    neck = BiFPN(BiFPN_inputs, 128, 'train')
...
if __name__=="__main__":
    ...
    #test = nn.ConvTranspose2d(3, 3, kernel_size=3, stride=2, padding=1)
    test = nn.MaxPool2d(kernel_size=3,stride=2,padding=1)
    print(test(y).shape)

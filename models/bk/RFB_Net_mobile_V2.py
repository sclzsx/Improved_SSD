...
def MobileNet():
    ...
    layers += [conv_dw(512, 512, 2)]
    layers += [conv_dw(512, 512, 1)]
    layers += [conv_dw(512, 512, 1)]
    layers += [conv_dw(512, 512, 1)]
    layers += [conv_dw(512, 1024, 1)]
...
def add_extras(size, cfg, i, batch_norm=False):
    ...
    if size ==300:
        ...
        layers += [BasicConv(64,128,kernel_size=3)]
    else:
        print("Error: Sorry only RFB300_mobile is supported!")
        return
    return layers
...
def multibox(size, base, extra_layers, cfg, num_classes):
    ...
    # 38x38  512
    loc_layers += [nn.Conv2d(512,cfg[0] * 4, kernel_size=1, padding=0)]
    conf_layers +=[nn.Conv2d(512,cfg[0] * num_classes, kernel_size=1, padding=0)]

    # 19x19  512
    loc_layers += [nn.Conv2d(1024,cfg[1] * 4, kernel_size=1, padding=0)]
    conf_layers +=[nn.Conv2d(1024,cfg[1] * num_classes, kernel_size=1, padding=0)]

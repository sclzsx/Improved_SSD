...
def build_net(phase, size=300, num_classes=21,neck_type='FPN'):
    ...
    backbone = resnet50(num_classes, pretrained=False)
    neck = PyramidFeatures(backbone.basic_channels * 2,
                           backbone.basic_channels * 4,
                           backbone.basic_channels * 8, feature_size=128, img_dim=size)
...
if __name__=='__main__':
    ...
    model = build_net('train', size=300)
    output = model(x)

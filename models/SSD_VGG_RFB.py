...
class SSD(nn.Module):
    ...
    def __init__(self, phase, backbone, neck, head, num_classes, img_dim=300):
        ...
        self.Norm = nn.ModuleList([BasicRFB(128,128,stride = 1,scale=1.0),
                                   BasicRFB(128,128,stride = 1,scale=1.0),
                                   BasicRFB(128,128,stride = 1,scale=1.0),
                                   BasicRFB(128,128,stride = 1,scale=1.0),
                                   BasicRFB(128,128,stride = 1,scale=1.0),
                                   BasicRFB(128,128,stride = 1,scale=1.0),])
    ...
    def forward(self, x):
        ...
        for (x, n, l, c) in zip(features, self.Norm, self.loc, self.conf):
            norm = n(x)
            loc.append(l(norm).permute(0, 2, 3, 1).contiguous())
            conf.append(c(norm).permute(0, 2, 3, 1).contiguous())
...
def add_extras(img_dim=300):
    ...
    if img_dim == 300:
        ...
        layers += [BasicConv(128,128,kernel_size=1,stride=1,padding=0)]
        layers += [BasicConv(128,128,kernel_size=3,stride=1, padding=0)] # 3 * 3

        layers += [BasicConv(128,128,kernel_size=1,stride=1,padding=0)]
        layers += [BasicConv(128,128,kernel_size=3,stride=1, padding=0)] # 1 * 1
    elif img_dim == 512:
        layers += [BasicConv(128,128,kernel_size=1,stride=1,padding=0)]

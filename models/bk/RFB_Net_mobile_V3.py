...
class RFBNet(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        #self.Norm = BasicRFB(512,512,stride = 1,scale=1.0)
    ...
    def forward(self, x):
        ...
        for k in range(8, len(self.base)):
            x = self.base[k](x)
        sources.append(x)
        #print(x.shape)
...
def MobileNet():
    base_channel = 8 * 2
    layers = []
    layers += [conv_bn(3, base_channel, 2)]  #256 * 256
    layers += [conv_dw(base_channel, base_channel * 2, 1)]
    layers += [conv_dw(base_channel * 2, base_channel * 2, 2)]  # 128*128
    layers += [conv_dw(base_channel * 2, base_channel * 2, 1)]
    layers += [conv_dw(base_channel * 2, base_channel * 4, 2)] # 64*64
    layers += [conv_dw(base_channel * 4, base_channel * 4, 1)]
    layers += [conv_dw(base_channel * 4, base_channel * 4, 1)]
    layers += [BasicRFB(base_channel * 4, base_channel * 4, stride=1, scale=1.0)]
    layers += [conv_dw(base_channel * 4, base_channel * 8, 2)] #32*32
    layers += [conv_dw(base_channel * 8, base_channel * 8, 1)]
    layers += [conv_dw(base_channel * 8, base_channel * 8, 1)]
    layers += [conv_dw(base_channel * 8, base_channel * 16, 1)] 
    layers += [conv_dw(base_channel * 16, base_channel * 16, 1)]
...
def add_extras(size, cfg, i, batch_norm=False):
    ...
    in_channels = i
    flag = False
    layers += [BasicRFB(256, 256, stride=2, scale=1.0)] # 16*16
    ...
    layers += [BasicConv(256,128,kernel_size=1,stride=1)]
    layers += [BasicConv(128,256,kernel_size=3,stride=1, padding=0)] # 2 * 2
    layers += [BasicConv(256,128,kernel_size=1,stride=1)]
    layers += [BasicConv(128,256,kernel_size=2,stride=1, padding=0)] # 1 * 1
...
def multibox(size, base, extra_layers, cfg, num_classes):
    ...
    # 64*64  512
    #loc_layers += [nn.Conv2d(64,cfg[0] * 4, kernel_size=1, padding=0)]
    #conf_layers +=[nn.Conv2d(64,cfg[0] * num_classes, kernel_size=1, padding=0)]

    # 32*32  512
    loc_layers += [nn.Conv2d(256,cfg[1] * 4, kernel_size=1, padding=0)]
    conf_layers +=[nn.Conv2d(256,cfg[1] * num_classes, kernel_size=1, padding=0)]
...
mbox = {
    ...
    '512': [6, 6, 6, 6, 6, 6],  # number of boxes per feature map location
}

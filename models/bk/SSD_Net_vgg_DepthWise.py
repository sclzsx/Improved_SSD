...
def MobileNetV1(cfg, i, batch_norm=False):
    base_channel = 32
    layers = []
    layers += [conv_bn(3, base_channel)]  
    layers += [conv_dw(base_channel, base_channel, 2)] #150 * 150

    layers += [conv_dw(base_channel, base_channel * 2, 1)]  
    layers += [conv_dw(base_channel * 2, base_channel * 2, 2)] #75 * 75

    layers += [conv_dw(base_channel * 2, base_channel * 4, 1)] 
    layers += [conv_dw(base_channel * 4, base_channel * 4, 1)]
    layers += [conv_dw(base_channel * 4, base_channel * 4, 2)] #38 * 38

    layers += [conv_dw(base_channel * 4, base_channel * 8, 1)] 
    layers += [conv_dw(base_channel * 8, base_channel * 8, 1)]
    layers += [conv_dw(base_channel * 8, base_channel * 8, 2)] # 19 * 19
...
def add_extras(cfg, i, batch_norm=False):
    ...
    layers += [BasicConv(128,256,kernel_size=3,stride=2, padding=1)] # 10*10
